...
/*Create columns for pickup and drop ZONES (using Localities data containing 
Zone IDs against each area) and fill corresponding values against pick-area 
and drop_area, using Sheet'Localities'?*/

WITH cte as(SELECT Booking_id,pickup_datetime,PickupArea ,zone_id
FROM `data` as d LEFT JOIN localities as l ON l.Area = d.PickupArea),
cte2 as (SELECT Booking_id,pickup_datetime,`DropArea` , zone_id
FROM `data` as d LEFT JOIN localities as l ON l.Area = d.`DropArea`)
SELECT cte.Booking_id,cte.pickup_datetime,cte.PickupArea ,cte.zone_id,
cte2.`DropArea` , cte2.zone_id
FROM cte join cte2 on cte.Booking_id = cte2.Booking_id;
